...
function t(e, s, n) {
    ...
    var i = e.b[n]
      ...
      , l = s[2] ^ i[2];
    s = s[n ? 1 : 3] ^ i[3];
    var h, u, m, g, p = i.length / 4 - 2, f = 4, S = [0, 0, 0, 0];
    e = (h = e.w[n])[0];
    var b = h[1]
      , y = h[2]
      , T = h[3]
    ...
    for (g = 0; g < p; g++)
        ...
        l = m;
    for (g = 0; 4 > g; g++)
        S[n ? 3 & -g : g] = I[r >>> 24] << 24 ^ I[o >> 16 & 255] << 16 ^ I[l >> 8 & 255] << 8 ^ I[255 & s] ^ i[f++],
        h = r,
        r = o,
        o = l,
        l = s,
...
function v(e) {
    e.b = w(e).concat(w(e)),
    e.I = new sjcl.cipher.aes(e.b)
}
function w(e) {
    for (var s = 0; 4 > s && (e.o[s] = e.o[s] + 1 | 0,
    !e.o[s]); s++)
        ;
...
sjcl.cipher.aes = function(e) {
    this.w[0][0][0] || this.L();
    var s, n, i, r, o = this.w[0][4], l = this.w[1], h = 1;
    if (4 !== (s = e.length) && 6 !== s && 8 !== s)
        throw new sjcl.exception.invalid("invalid aes key size");
    for (this.b = [i = e.slice(0), r = []],
    e = s; e < 4 * s + 28; e++)
        n = i[e - 1],
        (0 == e % s || 8 === s && 4 == e % s) && (n = o[n >>> 24] << 24 ^ o[n >> 16 & 255] << 16 ^ o[n >> 8 & 255] << 8 ^ o[255 & n],
        0 == e % s && (n = n << 8 ^ n >>> 24 ^ h << 24,
        h = h << 1 ^ 283 * (h >> 7))),
        i[e] = i[e - s] ^ n;
    for (s = 0; e; s++,
    e--)
        n = i[3 & s ? e : e - 4],
        r[s] = 4 >= e || 4 > s ? n : l[0][o[n >>> 24]] ^ l[1][o[n >> 16 & 255]] ^ l[2][o[n >> 8 & 255]] ^ l[3][o[255 & n]]
}
,
sjcl.cipher.aes.prototype = {
    ...
    w: [[[], [], [], [], []], [[], [], [], [], []]],
    L: function() {
        var e, s, n, i, r, o, l, h = this.w[0], u = this.w[1], m = h[4], g = u[4], p = [], f = [];
        for (e = 0; 256 > e; e++)
...
sjcl.bitArray = {
    bitSlice: function(e, s, n) {
        return e = sjcl.bitArray.X(e.slice(s / 32), 32 - (31 & s)).slice(1),
        void 0 === n ? e : sjcl.bitArray.clamp(e, n - s)
    },
    ...
    clamp: function(e, s) {
        ...
        return s &= 31,
        0 < n && s && (e[n - 1] = sjcl.bitArray.partial(s, e[n - 1] & 2147483648 >> s - 1, 1)),
        e
    },
    ...
    equal: function(e, s) {
        if (sjcl.bitArray.bitLength(e) !== sjcl.bitArray.bitLength(s))
            return !1;
        var n, i = 0;
    ...
    X: function(e, s, n, i) {
        ...
        for (r = 0; r < e.length; r++)
            i.push(n | e[r] >>> s),
            n = e[r] << 32 - s;
        return r = e.length ? e[e.length - 1] : 0,
        e = sjcl.bitArray.getPartial(r),
        i.push(sjcl.bitArray.partial(s + e & 31, 32 < s + e ? n : i.pop(), 1)),
        i
    },
    ka: function(e, s) {
    ...
    byteswapM: function(e) {
        ...
        for (s = 0; s < e.length; ++s)
            n = e[s],
            e[s] = n >>> 24 | n >>> 8 & 65280 | (65280 & n) << 8 | n << 24;
        return e
    }
},
sjcl.codec.utf8String = {
    fromBits: function(e) {
        var s, n, i = "", r = sjcl.bitArray.bitLength(e);
        for (s = 0; s < r / 8; s++)
            !(3 & s) && (n = e[s / 4]),
            i += String.fromCharCode(n >>> 8 >>> 8 >>> 8),
            n <<= 8;
    ...
    toBits: function(e) {
        ...
        var s, n = [], i = 0;
        for (s = 0; s < e.length; s++)
            i = i << 8 | e.charCodeAt(s),
            !(3 & ~s) && (n.push(i),
            i = 0);
        return 3 & s && n.push(sjcl.bitArray.partial(8 * (3 & s), i)),
        n
    }
},
sjcl.codec.hex = {
    fromBits: function(e) {
        var s, n = "";
        for (s = 0; s < e.length; s++)
            n += (0xf00000000000 + (0 | e[s])).toString(16).substr(4);
        return n.substr(0, sjcl.bitArray.bitLength(e) / 4)
    },
    toBits: function(e) {
        var s, n, i = [];
        for (n = (e = e.replace(/\s|0x/g, "")).length,
        e += "00000000",
        s = 0; s < e.length; s += 8)
            i.push(0 ^ parseInt(e.substr(s, 8), 16));
...
sjcl.codec.base64 = {
    ...
    toBits: function(e, s) {
        e = e.replace(/\s|=/g, "");
        var n, i, r = [], o = 0, l = sjcl.codec.base64.P, h = 0;
        for (s && (l = l.substr(0, 62) + "-_"),
        n = 0; n < e.length; n++) {
            if (0 > (i = l.indexOf(e.charAt(n))))
                throw new sjcl.exception.invalid("this isn't base64!");
            26 < o ? (o -= 26,
            r.push(h ^ i >>> o),
            h = i << 32 - o) : h ^= i << 32 - (o += 6)
        }
        return 56 & o && r.push(sjcl.bitArray.partial(56 & o, h, 1)),
        r
    }
},
...
sjcl.hash.sha256 = function(e) {
    this.b[0] || this.L(),
    e ? (this.i = e.i.slice(0),
    this.f = e.f.slice(0),
    this.c = e.c) : this.reset()
}
,
...
sjcl.hash.sha256.prototype = {
    ...
    update: function(e) {
        ...
        var s, n = this.f = sjcl.bitArray.concat(this.f, e);
        if (s = this.c,
        9007199254740991 < (e = this.c = s + sjcl.bitArray.bitLength(e)))
            throw new sjcl.exception.invalid("Cannot hash more than 2^53 - 1 bits");
        if ("undefined" != typeof Uint32Array) {
            var i = new Uint32Array(n)
              , r = 0;
            for (s = 512 + s - (512 + s & 511); s <= e; s += 512)
                this.m(i.subarray(16 * r, 16 * (r + 1))),
                r += 1;
            n.splice(0, 16 * r)
        } else
            for (s = 512 + s - (512 + s & 511); s <= e; s += 512)
                this.m(n.splice(0, 16));
        return this
    },
    finalize: function() {
        var e, s = this.f, n = this.i;
        for (e = (s = sjcl.bitArray.concat(s, [sjcl.bitArray.partial(1, 1)])).length + 2; 15 & e; e++)
            s.push(0);
        for (s.push(Math.floor(this.c / 4294967296)),
        s.push(0 | this.c); s.length; )
            this.m(s.splice(0, 16));
        return this.reset(),
        n
    },
    C: [],
    ...
    L: function() {
        ...
        for (var e, s, n = 0, i = 2; 64 > n; i++) {
            ...
            s && (8 > n && (this.C[n] = a(Math.pow(i, .5))),
            this.b[n] = a(Math.pow(i, 1 / 3)),
            n++)
        }
    },
    m: function(e) {
        var s, n, i, r = this.i, o = this.b, l = r[0], h = r[1], u = r[2], m = r[3], g = r[4], p = r[5], f = r[6], S = r[7];
        for (s = 0; 64 > s; s++)
            16 > s ? n = e[s] : (n = e[s + 1 & 15],
            i = e[s + 14 & 15],
...
sjcl.hash.sha1.prototype = {
    ...
    update: function(e) {
        ...
        if ("undefined" != typeof Uint32Array) {
            var i = new Uint32Array(n)
              , r = 0;
            for (s = this.blockSize + s - (this.blockSize + s & this.blockSize - 1); s <= e; s += this.blockSize)
                this.m(i.subarray(16 * r, 16 * (r + 1))),
                r += 1;
            n.splice(0, 16 * r)
        } else
    ...
    m: function(e) {
        ...
        l = u[4],
        e = 0; 79 >= e; e++)
            16 <= e && (s = h[e - 3] ^ h[e - 8] ^ h[e - 14] ^ h[e - 16],
            h[e] = s << 1 | s >>> 31),
...
sjcl.mode.gcm = {
    ...
    encrypt: function(e, s, n, i, r) {
        var o = s.slice(0);
        return s = sjcl.bitArray,
        i = i || [],
        e = sjcl.mode.gcm.S(!0, e, o, i, n, r || 128),
        s.concat(e.data, e.tag)
    },
    decrypt: function(e, s, n, i, r) {
        var o = s.slice(0)
          ...
          , h = l.bitLength(o);
        if (i = i || [],
        (r = r || 128) <= h ? (s = l.bitSlice(o, h - r),
        o = l.bitSlice(o, 0, h - r)) : (s = o,
        o = []),
        e = sjcl.mode.gcm.S(!1, e, o, i, n, r),
        !l.equal(e.tag, s))
            throw new sjcl.exception.corrupt("gcm: tag doesn't match");
        return e.data
    },
    ea: function(e, s) {
        var n, i, r, o, l, h = sjcl.bitArray.ka;
        for (r = [0, 0, 0, 0],
        o = s.slice(0),
        n = 0; 128 > n; n++) {
            for ((i = !!(e[Math.floor(n / 32)] & 1 << 31 - n % 32)) && (r = h(r, o)),
            l = !!(1 & o[3]),
            i = 3; 0 < i; i--)
                o[i] = o[i] >>> 1 | (1 & o[i - 1]) << 31;
            o[0] >>>= 1,
            l && (o[0] ^= -520093696)
        }
        return r
    ...
    s: function(e, s, n) {
        ...
        i = 0; i < r; i += 4)
            s[0] ^= 4294967295 & n[i],
            s[1] ^= 4294967295 & n[i + 1],
            s[2] ^= 4294967295 & n[i + 2],
            s[3] ^= 4294967295 & n[i + 3],
            s = sjcl.mode.gcm.ea(s, e);
        return s
    },
    S: function(e, s, n, i, r, o) {
        var l, h, u, m, g, p, f, S, b = sjcl.bitArray;
        for (p = n.length,
        f = b.bitLength(n),
        S = b.bitLength(i),
        h = b.bitLength(r),
        l = s.encrypt([0, 0, 0, 0]),
        96 === h ? (r = r.slice(0),
        r = b.concat(r, [1])) : (r = sjcl.mode.gcm.s(l, [0, 0, 0, 0], r),
        r = sjcl.mode.gcm.s(l, r, [0, 0, Math.floor(h / 4294967296), 4294967295 & h])),
...
sjcl.prng.prototype = {
    randomWords: function(e, s) {
        var n, i, r = [];
        if ((n = this.isReady(s)) === this.A)
            throw new sjcl.exception.notReady("generator isn't seeded");
        if (n & this.G) {
            n = !(n & this.F),
            i = [];
            var o, l = 0;
            for (this.W = i[0] = (new Date).valueOf() + this.aa,
            o = 0; 16 > o; o++)
                i.push(4294967296 * Math.random() | 0);
            for (o = 0; o < this.j.length && (i = i.concat(this.j[o].finalize()),
            l += this.u[o],
            this.u[o] = 0,
            n || !(this.M & 1 << o)); o++)
                ;
            for (this.M >= 1 << this.j.length && (this.j.push(new sjcl.hash.sha256),
            this.u.push(0)),
            this.l -= l,
            l > this.v && (this.v = l),
            this.M++,
    ...
    setDefaultParanoia: function(e, s) {
        if (0 === e && "Setting paranoia=0 will ruin your security; use it only for testing" !== s)
    ...
    addEntropy: function(e, s, n) {
        n = n || "user";
        var i, r, o = (new Date).valueOf(), l = this.D[n], h = this.isReady(), u = 0;
        switch (void 0 === (i = this.R[n]) && (i = this.R[n] = this.da++),
        void 0 === l && (l = this.D[n] = 0),
        this.D[n] = (this.D[n] + 1) % this.j.length,
        typeof e) {
        case "number":
            void 0 === s && (s = 1),
            this.j[l].update([i, this.K++, 1, s, o, 1, 0 | e]);
            break;
        case "object":
        ...
        case "string":
            void 0 === s && (s = e.length),
            this.j[l].update([i, this.K++, 3, s, o, e.length]),
            this.j[l].update(e);
            break;
        default:
            u = 1
        ...
        this.u[l] += s,
        this.l += s,
        h === this.A && (this.isReady() !== this.A && z("seeded", Math.max(this.v, this.l)),
        z("progress", this.getProgress()))
    },
    isReady: function(e) {
        return e = this.O[void 0 !== e ? e : this.J],
        this.v && this.v >= e ? this.u[0] > this.Z && (new Date).valueOf() > this.W ? this.G | this.F : this.F : this.l >= e ? this.G | this.A : this.A
    },
    getProgress: function(e) {
        return e = this.O[e || this.J],
        this.v >= e || this.l > e ? 1 : this.l / e
    },
    ...
    stopCollectors: function() {
        this.B && (window.removeEventListener ? (window.removeEventListener("load", this.a.loadTimeCollector, !1),
    ...
    removeEventListener: function(e, s) {
        var n, i, r = this.H[e], o = [];
        for (i in r)
            r.hasOwnProperty(i) && r[i] === s && o.push(i);
    ...
    ia: function(e) {
        var s, n;
        try {
            s = e.x || e.clientX || e.offsetX || 0,
            n = e.y || e.clientY || e.offsetY || 0
        } catch (e) {
            n = s = 0
        }
        0 != s && 0 != n && this.addEntropy([s, n], 2, "mouse"),
        B(this, 0)
    },
    ja: function(e) {
...
sjcl.bn.prototype = {
    ...
    initWith: function(e) {
        ...
        case "string":
            ...
            s = this.radix / 4,
            n = 0; n < e.length; n += s)
                this.limbs.push(parseInt(e.substring(Math.max(e.length - n - s, 0), e.length - n), 16));
            break;
        default:
            this.limbs = [0]
        }
    ...
    equals: function(e) {
        ...
        for (this.fullReduce(),
        e.fullReduce(),
        s = 0; s < this.limbs.length || s < e.limbs.length; s++)
            n |= this.getLimb(s) ^ e.getLimb(s);
        return 0 === n
    },
    getLimb: function(e) {
    ...
    greaterEquals: function(e) {
        "number" == typeof e && (e = new this.h(e));
        var s, n, i, r = 0, o = 0;
        for (s = Math.max(this.limbs.length, e.limbs.length) - 1; 0 <= s; s--)
            r |= (n = this.getLimb(s)) - (i = e.getLimb(s)) & ~(o |= i - n & ~r);
        return (o | ~r) >>> 31
    },
    toString: function() {
        this.fullReduce();
        var e, s, n = "", i = this.limbs;
        for (e = 0; e < this.limbs.length; e++) {
            for (s = i[e].toString(16); e < this.limbs.length - 1 && 6 > s.length; )
                s = "0" + s;
            n = s + n
        }
        return "0x" + n
    ...
    doubleM: function() {
        var e, s, n = 0, i = this.radix, r = this.radixMask, o = this.limbs;
    ...
    halveM: function() {
        var e, s, n = 0, i = this.radix, r = this.limbs;
    ...
    mod: function(e) {
        ...
        var n = new sjcl.bn(this).normalize()
          , i = 0;
        for (s && (n = new sjcl.bn(0).subM(n).normalize()); n.greaterEquals(e); i++)
            e.doubleM();
        for (s && (n = e.sub(n).normalize()); 0 < i; i--)
            e.halveM(),
            n.greaterEquals(e) && n.subM(e).normalize();
        return n.trim()
    },
    inverseMod: function(e) {
        var s, n = new sjcl.bn(1), i = new sjcl.bn(0), r = new sjcl.bn(this), o = new sjcl.bn(e), l = 1;
        if (!(1 & e.limbs[0]))
            throw new sjcl.exception.invalid("inverseMod: p must be odd");
        do {
            for (1 & r.limbs[0] && (r.greaterEquals(o) || (s = r,
            r = o,
            o = s,
            s = n,
            n = i,
            ...
            r.subM(o),
            r.normalize(),
            n.greaterEquals(i) || n.addM(e),
            n.subM(i)),
            r.halveM(),
            1 & n.limbs[0] && n.addM(e),
            n.normalize(),
            n.halveM(),
            s = l = 0; s < r.limbs.length; s++)
                l |= r.limbs[s]
        } while (l);
        if (!o.equals(1))
    ...
    mul: function(e) {
        "number" == typeof e ? e = new this.h(e) : e.normalize(),
        this.normalize();
        var s, n, i = this.limbs, r = e.limbs, o = i.length, l = r.length, h = new this.h, u = h.limbs, m = this.maxMul;
        for (s = 0; s < this.limbs.length + e.limbs.length + 1; s++)
            u[s] = 0;
        for (s = 0; s < o; s++) {
            for (n = i[s],
            e = 0; e < l; e++)
    ...
    power: function(e) {
        ...
        var s, n, i = new this.h(1), r = this;
        for (s = 0; s < e.length; s++)
            for (n = 0; n < this.radix && (e[s] & 1 << n && (i = i.mul(r)),
            s != e.length - 1 || e[s] >> n + 1); n++)
                r = r.square();
        return i
    },
    mulmod: function(e, s) {
    ...
    powermod: function(e, s) {
        if ((e = new sjcl.bn(e),
        !(1 & ~(s = new sjcl.bn(s)).limbs[0])) && 0 != (l = this.montpowermod(e, s)))
            return l;
        for (var n, i = e.normalize().trim().limbs, r = new this.h(1), o = this, l = 0; l < i.length; l++)
            for (n = 0; n < this.radix && (i[l] & 1 << n && (r = r.mulmod(o, s)),
            l != i.length - 1 || i[l] >> n + 1); n++)
                o = o.mulmod(o, s);
        return r
    },
    montpowermod: function(e, s) {
        ...
        function d(e, n) {
            var i, l, u = (1 << o + 1) - 1;
            for ((l = (i = e.mul(n)).mul(p)).limbs = l.limbs.slice(0, r.limbs.length),
            l.limbs.length == r.limbs.length && (l.limbs[r.limbs.length - 1] &= u),
            l = l.mul(s),
            (l = i.add(l).normalize().trim()).limbs = l.limbs.slice(r.limbs.length - 1),
            i = 0; i < l.limbs.length; i++)
                0 < i && (l.limbs[i - 1] |= (l.limbs[i] & u) << h - o - 1),
                l.limbs[i] >>= o + 1;
            return l.greaterEquals(s) && l.subM(s),
            l
        }
        e = new sjcl.bn(e).normalize().trim(),
        s = new sjcl.bn(s);
        var n, i, r, o, l, h = this.radix, u = new this.h(1);
        for (n = this.copy(),
        l = e.bitLength(),
        r = new sjcl.bn({
        ...
        var m = r.copy()
          , g = s.copy();
        i = new sjcl.bn(1);
        for (var p = new sjcl.bn(0), f = r.copy(); f.greaterEquals(1); )
            f.halveM(),
            1 & i.limbs[0] ? (i.addM(g),
            i.halveM(),
            p.halveM(),
            p.addM(m)) : (i.halveM(),
            p.halveM());
    ...
    normalize: function() {
        var e, s = 0, n = this.placeVal, i = this.ipv, r = this.limbs, o = r.length, l = this.radixMask;
        for (e = 0; e < o || 0 !== s && -1 !== s; e++)
            s = ((s = (r[e] || 0) + s) - (r[e] = s & l)) * i;
        return -1 === s && (r[e - 1] -= n),
        this.trim(),
        this
    },
    cnormalize: function() {
        var e, s = 0, n = this.ipv, i = this.limbs, r = i.length, o = this.radixMask;
        for (e = 0; e < r - 1; e++)
            s = ((s = i[e] + s) - (i[e] = s & o)) * n;
        return i[e] += s,
        this
    ...
    toBits: function(e) {
        this.fullReduce(),
        e = e || this.exponent || this.bitLength();
        var s = Math.floor((e - 1) / 24)
          , n = sjcl.bitArray
          , i = [n.partial((e + 7 & -8) % this.radix || this.radix, this.getLimb(s))];
        for (s--; 0 <= s; s--)
            i = n.concat(i, [n.partial(Math.min(this.radix, e), this.getLimb(s))]),
            e -= this.radix;
        return i
    },
    bitLength: function() {
        this.fullReduce();
        for (var e = this.radix * (this.limbs.length - 1), s = this.limbs[this.limbs.length - 1]; s; s >>>= 1)
            e++;
        return e + 7 & -8
    }
},
sjcl.bn.fromBits = function(e) {
    var s = new this
      ...
      , l = o % r.radix || r.radix;
    for (n[0] = i.extract(e, 0, l); l < o; l += r.radix)
        n.unshift(i.extract(e, l, r.radix));
    return s.limbs = n,
    s
}
,
...
sjcl.bn.pseudoMersennePrime = function(e, s) {
    ...
    var n, i, r, o = c.prototype = new sjcl.bn;
    for (r = o.modOffset = Math.ceil(i = e / o.radix),
    o.exponent = e,
    o.offset = [],
    o.factor = [],
    o.minOffset = r,
    ...
    n = 0; n < s.length; n++)
        ...
        o.factor[n] = s[n][1] * Math.pow(.5, e - s[n][0] + o.offset[n] * o.radix),
        o.fullFactor[n] = s[n][1] * Math.pow(.5, e - s[n][0] + o.fullOffset[n] * o.radix),
        o.modulus.addM(new sjcl.bn(Math.pow(2, s[n][0]) * s[n][1])),
        o.minOffset = Math.min(o.minOffset, -o.offset[n]);
    return o.h = c,
    o.modulus.cnormalize(),
    o.reduce = function() {
        var e, s, n, i, r = this.modOffset, o = this.limbs, l = this.offset, h = this.offset.length, u = this.factor;
    ...
    ,
    o.Y = -1 === o.fullMask ? o.reduce : function() {
        var e, s, n = this.limbs, i = n.length - 1;
        if (this.reduce(),
        i === this.modOffset - 1) {
            for (s = n[i] & this.fullMask,
            n[i] -= s,
            e = 0; e < this.fullOffset.length; e++)
                n[i + this.fullOffset[e]] -= this.fullFactor[e] * s;
            this.normalize()
        }
    }
    ,
    o.fullReduce = function() {
        ...
        this.normalize(),
        this.Y(),
        s = this.limbs.length; s < this.modOffset; s++)
            this.limbs[s] = 0;
        for (e = this.greaterEquals(this.modulus),
        s = 0; s < this.limbs.length; s++)
            this.limbs[s] -= this.modulus.limbs[s] * e;
        return this.cnormalize(),
        this
    }
    ,
...
sjcl.bn.prime = {
    ...
    p256k: H(256, [[32, -1], [9, -1], [8, -1], [7, -1], [6, -1], [4, -1], [0, -1]]),
    p192: H(192, [[0, -1], [64, -1]]),
    p224: H(224, [[0, 1], [96, -1]]),
    p256: H(256, [[0, -1], [96, 1], [192, 1], [224, -1]]),
...
sjcl.keyexchange.srp = {
    makeVerifier: function(e, s, n, i) {
        return e = sjcl.keyexchange.srp.makeX(e, s, n),
        e = sjcl.bn.fromBits(e),
        i.g.powermod(e, i.N)
    },
    makeX: function(e, s, n) {
        return e = sjcl.hash.sha1.hash(e + ":" + s),
        sjcl.hash.sha1.hash(sjcl.bitArray.concat(n, e))
    },
    knownGroup: function(e) {
        return "string" != typeof e && (e = e.toString()),
        sjcl.keyexchange.srp.T || sjcl.keyexchange.srp.fa(),
        sjcl.keyexchange.srp.V[e]
    },
    T: !1,
    fa: function() {
        var e, s;
        for (e = 0; e < sjcl.keyexchange.srp.U.length; e++)
            s = sjcl.keyexchange.srp.U[e].toString(),
            (s = sjcl.keyexchange.srp.V[s]).N = new sjcl.bn(s.N),
            s.g = new sjcl.bn(s.g);
...
class Localizer {
    ...
    messageNamesToTry(e) {
        ...
        const n = this.#e
          ...
          , l = void 0 !== r;
        return i && l && void 0 !== o && s.push(`${e}_${i}_${r}_${o}`),
        i && l && s.push(`${e}_${i}_${r}`),
        i ? s.push(`${e}_${i}`) : s.push(`${e}_${this.#t}`),
        s.push(e),
        s
    }
    get #t() {
...
class ExtensionSettings {
    ...
    #r() {
        let e = new Promise((e => {
            chrome.storage.sync.get({
                ...
                this.#n = s.enableInPageAutoFill,
                this.#i = s.allowExtensionToControlAutoFillSettings,
                e()
            }
            ))
        }
        ));
        return this.#s || (e = e.then(this.#c.bind(this))),
        e.then(this.#d.bind(this))
    }
    #a() {
    ...
    async #u(e) {
        ...
        const s = await e.get({});
        if ("not_controllable" === s.levelOfControl)
...
SecretSession.prototype = {
    ...
    parseSMSG: function(e) {
        ...
        if ("string" != typeof s.SDATA)
            throw new SecretSessionError(ErrCodes.InvalidMessage,"Missing or invalid SDATA field in SMSG message.");
        if ("string" != typeof s.TID)
            throw new SecretSessionError(ErrCodes.InvalidUserName,"Missing or invalid 'TID' field in SMSG object.");
        const n = sjcl.bn.fromBits(this.stringToBits(s.TID));
        if (!this.I.equals(n))
            throw new SecretSessionError(ErrCodes.InvalidUserName,"Received SMSG message meant for another session.");
    ...
    setUpSRP: function() {
        this.I = sjcl.bn.fromBits(this._randomWords(4)),
        this.a = sjcl.bn.fromBits(this._randomWords(8)),
        this.A = this.grp.g.powermod(this.a, this.grp.N)
    },
    ...
    processMessage: function(e) {
        ...
        if ("string" != typeof n.TID)
            throw new SecretSessionError(ErrCodes.InvalidMessage,"Missing or invalid 'TID' field in PAKE message.");
        let i = sjcl.bn.fromBits(this.stringToBits(n.TID));
        if (!this.I.equals(i))
            throw new SecretSessionError(ErrCodes.UnexpectedMessage,"Unexpected message");
        if (!n.MSG)
            throw new SecretSessionError(ErrCodes.InvalidMessage,"Missing 'MSG' field in PAKE message.");
        const r = parseInt(n.MSG, 10);
        if (this.expectedMessage !== r)
            throw new SecretSessionError(ErrCodes.UnexpectedMessage,`Received Message ${message.MSG}, but expected Message ${this.expectedMessage} `);
        let o = null;
        if (r === MSGTypes.MSG1) {
            var l = SecretSessionVersion.SRPWithOldVerification;
            "number" == typeof n.PROTO && (l = n.PROTO,
            Object.values(SecretSessionVersion).includes(l) || (l = SecretSessionVersion.SRPWithOldVerification)),
            this.protocolVersion = l,
            o = this.processMessage1(n)
        } else
            r === MSGTypes.MSG3 && (o = this.processMessage3(n));
        return o
    },
    ...
    _calculateM: function(e, s, n) {
        ...
        u.update(h),
        u.update(e),
        u.update(this.A.toBits()),
        u.update(s.toBits()),
        u.update(n);
        let m = u.finalize();
        return u = new sjcl.hash.sha256,
        u.update(this.A.toBits()),
        u.update(m),
        u.update(n),
        this.hamk = u.finalize(),
        m
    },
    processMessage1: function(e) {
        if ("string" != typeof e.s || "string" != typeof e.B)
            throw new SecretSessionError(ErrCodes.InvalidMessage,"Message 1 is missing some required keys.");
        e.VER && (this.appVer = e.VER);
        const s = this.stringToBits(e.s)
          , n = sjcl.bn.fromBits(this.stringToBits(e.B));
        let i = new sjcl.bn(1);
        if (0 === n.mulmod(i, this.grp.N))
            throw new SecretSessionError(ErrCodes.CryptoError,"B.mulmod error");
        const r = this.createSessionKey(s, n);
        this.encKey = sjcl.bitArray.bitSlice(r, 0, this.keyLen),
        this.expectedMessage = MSGTypes.MSG3;
        let o = {
            TID: this.bitsToString(this.I.toBits()),
            MSG: 2
        };
    ...
    processMessage3: function(e) {
        let s = ErrCodes.ErrSuccess
          , n = "";
        if (0 !== e.ErrCode)
            n = `Message 3 contained an error: ${e.ErrCode}`,
            s = ErrCodes.InvalidMessage;
        else
            switch (this.protocolVersion) {
            case SecretSessionVersion.SRPWithRFCVerification:
                if (e.HAMK) {
                    const i = this.stringToBits(e.HAMK);
                    sjcl.bitArray.equal(i, this.hamk) || (n = "Failed to verify server data.",
                    this.msgExp = MSGTypes.MSG1,
                    s = ErrCodes.InvalidSessionKey)
                } else
                    n = `Message 3 does not contain necessary data:${e.ErrCode}`,
                    s = ErrCodes.InvalidMessage;
                break;
            case SecretSessionVersion.SRPWithOldVerification:
                break;
            default:
                s = ErrCode.UnexpectedMessage,
                n = `Unknown SecretSessionVersion ${this.protocolVersion}.`
            }
        if (s !== ErrCodes.ErrSuccess)
            throw this.sessionKey = null,
            this.expectedMessage = MSGTypes.MSG1,
            new SecretSessionError(s,n);
        return s === ErrCodes.ErrSuccess
    },
    encrypt: function(e) {
        if (!this.encKey)
            throw new SecretSessionError(ErrCodes.InvalidSessionKey,"Called encrypt() without a session key");
    ...
    decrypt: function(e) {
        if (!this.encKey)
            throw new SecretSessionError(ErrCodes.InvalidSessionKey,"Called decrypt() without a session key!");
        const s = new sjcl.cipher.aes(this.encKey)
    ...
    _randomWords: function(e) {
        ...
        return self.crypto.getRandomValues(s),
        Array.from(s)
    }
};
const ContextState = {
...
class FrameDoesNotExistError extends Error {
    frame;
    constructor(e) {
        super(`Frame ${e.id} does not exist in tab ${e.tab.id}`),
        this.frame = e
    }
}
class FrameIsBrowserFrameError extends Error {
    frame;
    constructor(e) {
        super(`Frame ${e.id} in tab ${e.tab.id} is a browser frame.`),
        this.frame = e
    }
}
class ContentScriptNotSetUpInFrame extends Error {
    frame;
    constructor(e) {
        super(`Frame ${e.id} in tab ${e.tab.id} does not have active content script.`),
        this.frame = e
    }
}
...
class DelayedMessageSender {
    ...
    enqueueRequest(e, s) {
        return new Promise(( (n, i) => {
            ...
            this.#m = null),
            this.#m = setTimeout((async () => {
                const n = e.tab.id
                  , i = e.id;
                let r;
                try {
                    r = await this.#f(n, i, this.messageName, s)
                } catch (s) {
                    let n = s;
                    return s.message.includes("Receiving end does not exist.") && (n = new FrameDoesNotExistError(e)),
                    this.#p = null,
                    void this.#C(null, n)
                } finally {
                    this.#m = null
    ...
    cancelPendingRequests() {
        this.#m && (clearTimeout(this.#m),
        this.#m = null),
        this.#C(null, new Error("Cancelled"))
    }
    reset() {
...
class Frame {
    ...
    setUpContentScript(e) {
        return this.isBrowserFrame ? Promise.reject(new FrameIsBrowserFrameError(this)) : (e = !!e,
        this.#F.enqueueRequest(this, {
            shouldEnableQRCodeScanning: e
        }))
    }
    async enableQRCodeScanning(e) {
        return this.hasSetUpContentScript ? this.#f("enableQRCodeScanner", {
            isEnabled: e
    ...
    #f(e, s) {
        ...
        chrome.tabs.sendMessage(this.tab.id, n, {
            frameId: this.id
        })
    }
}
class Tab {
    ...
    async injectContentScripts(e=!0) {
        ...
        for (const e of this.framesById.values())
            try {
                ...
                if (e instanceof FrameIsBrowserFrameError)
                    return
            }
    }
    prepareForRemoval() {
    ...
    update(e, s) {
        this.#b(),
        this.state = s.status,
        e ? (this.state = e.status,
        "loading" === e.status && (this.url = urlFromURLString(e.url))) : (this.state = s.status,
        this.url = urlFromURLString(s.url))
    }
    addNewFrame(e, s, n=null) {
        this.#b();
        const i = s.frameId
          , r = new Frame(i,this);
        this.framesById.set(i, r),
        this.frameUpdated(e, i, n, s)
    }
    async frameUpdated(e, s, n, i) {
        this.#b();
        const r = this.ensureFrameExists(s);
        if (r.updateWithDetails(n, i),
        r.hasCompletedLoad)
            try {
                ...
                {
                    const s = e.frameId;
                    if (!this.framesById.get(s))
                        return;
                    let n = await chrome.webNavigation.getFrame({
                        tabId: this.id,
                        frameId: s
                    });
                    n || (n.prepareForRemoval(),
                    this.framesById.delete(s))
                }
            }
    }
    ...
    ensureFrameExists(e) {
        ...
        return s || (s = new Frame(e,this),
        this.framesById.set(e, s)),
        s
    }
    #b() {
        Tab.enableDeferredFrameCleanup && (this.#A && (clearTimeout(this.#A),
        this.#A = null),
        this.#A = setTimeout((async () => {
            let e = new Map;
            const s = await this.allFrames
              , n = new Set((s || []).map((e => e.frameId)));
            if (!n.size && !this.framesById.size)
                return;
            for (const [s,i] of this.framesById)
                n.has(s) && e.set(s, i);
...
class TabMonitor {
    ...
    async #D() {
        ...
        for (const s of e) {
            const e = s.id
              , n = new Tab(e);
            n.update(null, s),
            this.tabsById.set(e, n);
            const i = await chrome.webNavigation.getAllFrames({
                tabId: e
            });
            for (const e of i) {
                if (void 0 === e.frameId)
    ...
    #w() {
        ...
        chrome.webNavigation.onTabReplaced.addListener((e => {
            ...
            this.tabsById.get(s).prepareForRemoval(),
            this.tabsById.delete(s);
            const n = e.tabId;
    ...
    async performInitialSetup() {
        for (const e of this.tabsById.values())
            await e.injectContentScripts()
    }
    ensureTabExists(e) {
        let s = this.tabsById.get(e);
        return s || (s = new Tab(e),
        this.tabsById.set(e, s)),
        s
    }
...
function imageDataForName(e) {
    if (e)
        return {
            16: "images/" + (g_lastToolbarIconImageName = e) + (getIsDark() ? "-darkmode" : "") + "_icon16.png",
            32: "images/" + g_lastToolbarIconImageName + (getIsDark() ? "-darkmode" : "") + "_icon32.png"
        }
}
function setUpEventListners() {
    chrome.runtime.onInstalled && chrome.runtime.onInstalled.addListener((async e => {
        ...
        "install" === e.reason && g_extensionSettings.attemptToControlBrowserAutoFillSettings().then((e => {}
        )).catch((e => {}
        ))
    }
    )),
    chrome.runtime.onSuspend && chrome.runtime.onSuspend.addListener((function() {
        g_tabIdToURL.clear(),
        g_Stage1Logins.clear(),
        g_TabsToStateMap.clear(),
...
function checkForValidOS() {
    if ("MacIntel" === navigator.platform)
        return g_urlToDownloadNativeSupport = "https://www.apple.com/macos",
        !0;
    const e = new RegExp("\\(Windows\\s*\\w*\\s*(\\d*)\\.(\\d*)","i").exec(navigator.userAgent);
    return null !== e && 3 === e.length && (e[1] >= 10 ? (g_urlToDownloadNativeSupport = "ms-windows-store://pdp/?productid=9PKTQ5699M62",
    !0) : !(e[1] < 6) && !(e[2] < 1))
}
function setGlobalState(e, s) {
    g_theState = e,
    g_tabMonitor.extensionIsPaired = g_theState === ContextState.SessionKeySet,
    setToolbarIcon(g_theState === ContextState.SessionKeySet ? "PasswordsToolbar" : "PasswordsToolbarUnpaired"),
...
function consultStage1Logins(e, s) {
    return isStringEmpty(e) ? g_Stage1Logins.has(s) ? (LoginName = g_Stage1Logins.get(s),
    LoginName,
    g_Stage1Logins.delete(s),
    LoginName) : "" : e
}
function entriesFromLoginNames4URLData(e) {
    let s = e.Entries;
    return s || (s = [],
    Object.entries(e).sort().map(( ([e,n]) => {
...
function onSetUpTOTPContextMenuClicked(e, s) {
    if (g_theState !== ContextState.SessionKeySet)
        window.alert(g_localizer.getMessage("enableAutoFillPasswordsMessage"));
    else {
        const s = new URL(e.pageUrl);
        s.hostname,
        e.menuItemId,
        setUpTOTP(s.hostname, e.menuItemId)
    }
}
function connectToBackgroundNativeAppAndSetUpListeners() {
    ...
    chrome.runtime.onMessage.addListener(( (e, s, n) => {
        ...
        "content" === e.from)
            ...
            case "SaveStage1LoginName":
                g_tabIdToURL.has(i) && (strURL = g_tabIdToURL.get(i),
                g_Stage1Logins.delete(strURL),
                g_tabIdToURL.delete(i)),
                o.hostname,
                e.theLogin,
                g_tabIdToURL.set(i, o.hostname),
                g_Stage1Logins.set(o.hostname, e.theLogin);
                break;
            ...
            case "CmdSetIconNTitle":
                {
                    ...
                    switch (g_TabsToStateMap.has(s.tab.id) && g_TabsToStateMap.get(s.tab.id) || g_TabsToStateMap.set(s.tab.id, i),
                    o.hostname,
                    s.tab.id,
                    s.frameId,
                    humanReadableFormType(e.hostPageType),
                    e.hostPageType,
                    e.hostPageType) {
                    case WBSAutoFillFormTypeUndetermined:
                    case WBSAutoFillFormTypeAutoFillableStandard:
                    ...
                    case WBSAutoFillFormTypeChangePassword:
                        ...
                        JSON.stringify(e),
                        g_nativeAppPort.postMessage(e)
                    }
                }
                break;
            case "ThemeChanged":
                setIsDark(e.isDark),
                setToolbarIcon(g_lastToolbarIconImageName);
                break;
            case "fillOneTimeCodeIntoForm":
                chrome.webNavigation.getFrame({
                    ...
                    n.frameId = s.frameId,
                    didFillOneTimeCode(e.oneTimeCode, {
                        id: s.tab.id
                    }, n)
                }
                ));
            ...
            case "CmdAddSetUpTOTPContextMenu":
                ...
                chrome.contextMenus.create({
                    title: m ? `${g_localizer.getMessage("divTOTPMenu")} (${e.totpSetupURL})` : g_localizer.getMessage("divTOTPMenu"),
                    id: e.totpSetupURL,
                    contexts: ["all"]
                }),
    ...
    try {
        (g_nativeAppPort = chrome.runtime.connectNative("com.apple.passwordmanager")).onMessage.addListener((async function(e) {
            switch (JSON.stringify(e),
            chrome.storage.local.remove(["lastRetryTimestamp"]),
            cmd2string(e.cmd),
            e.cmd,
            e.cmd) {
            case 9:
            case 10:
                ...
                break;
            case 2:
                switch (e.payload.QID,
                e.payload.QID) {
                case "m0":
                    thePAKE = e.payload.PAKE,
                    setGlobalState(ContextState.MSG1Set);
                    break;
                case "m2":
                    try {
                        g_secretSession.processMessage(e.payload.PAKE);
                        setGlobalState(ContextState.SessionKeySet),
                        thePAKE = null
                    } catch (e) {
                        e.code,
                        e.message,
            ...
            case 3:
                ...
                case "UnknownPage":
                    ...
                    break;
                case "NoValueSet":
                    s = RememberIC.NoValueSet,
                    STATUSErrorReturned(e.STATUS)
                }
                try {
                ...
                case QueryStatus.Success:
                    {
                        ...
                        entriesFromLoginNames4URLData(m).forEach((e => {
                            if ("Passwords not saved" === e.USR)
                                s = RememberIC.DoNotRemember;
                            else if (n.push(e.USR),
                            i.push(e.CDate || e.ModDate),
                            r.push(e.sites[0]),
                            "Not Included" === e.PWD)
                                s = RememberIC.RememberLoginAndPassword;
                            else
                        ...
                        sendMessageToPopupAndCompletionList({
                            ...
                            frameId: h,
                            theRememberICSelection: s,
                            requiresUserAuthenticationToFill: e
                        });
                        break
                    }
        ...
        g_nativeAppPort.onDisconnect.addListener((e => {
            ...
            setTimeout(( () => {
                if (s.message,
                "Native host has exited." === s.message)
                    chrome.storage.local.get(["lastRetryTimestamp"], (e => {
                        ...
                        connectToBackgroundNativeAppAndSetUpListeners()) : (chrome.storage.local.remove(["lastRetryTimestamp"]),
...
function handleGetOneTimeCodesCommand(e) {
    ...
    let i = s.STATUS;
    if (i !== QueryStatus.Success)
        return i !== QueryStatus.NoResults && STATUSErrorReturned(i),
        void sendMessageToPopupAndCompletionList({
            from: "background",
            subject: "oneTimeCodes",
            oneTimeCodes: [],
            requiresUserAuthenticationToFill: n
        });
    let r = s.Entries;
    r && sendMessageToPopupAndCompletionList({
        from: "background",
        subject: "oneTimeCodes",
        oneTimeCodes: r,
        requiresUserAuthenticationToFill: n
...
async function handleDidFillOneTimeCodeCommand(e) {
    ...
    let n = s.STATUS;
    if (n !== QueryStatus.Success)
        return void (n !== QueryStatus.NoResults && STATUSErrorReturned(n));
    let i = s.Entries;
    if (i && i.length)
        try {
            await chrome.tabs.sendMessage(e.tabId, {
...
function setUpTOTP(e, s) {
    if (!g_nativeAppCapabilities.scanForOTPURI)
        return;
    const n = {
        cmd: 13,
        setUpTOTPPageURL: e,
...
function GetLoginNames4URL(e, s, n) {
    ...
    g_timeStartedFetchingCredentials = performance.now(),
    g_nativeAppPort.postMessage(o)
}
function getOneTimeCodes(e, s, n) {
    getAllParentFrameURLsOfFrame(e, s).then((i => {
        let r = {
            ACT: actGhostSearch,
            TYPE: "oneTimeCodes",
            frameURLs: i
...
function didFillOneTimeCode(e, s, n) {
    if ("totp" === e.source)
        getAllParentFrameURLsOfFrame(s, n).then((i => {
            let r = {
                ACT: actSearch,
                TYPE: "oneTimeCodes",
                frameURLs: i
...
function getAllParentFrameURLsOfFrame(e, s) {
    ...
    return new Promise((i => {
        -1 !== s.parentFrameId ? chrome.webNavigation.getFrame({
            tabId: n,
            frameId: s.parentFrameId
        }, (n => {
            getAllParentFrameURLsOfFrame(e, n).then((e => {
                i([s.url].concat(e))
            }
            )).catch((e => {}
            ))
...
function setUpPortToCompetionList(e) {
    ...
    e.onMessage.addListener((async function(s) {
        ...
        case "getContextAndMetadataFromContent":
            getCurrentActiveTabAndItsFrames().then((function(s) {
                const n = s[0].id
                  , i = s[0].incognito;
                for (const r of s[1]) {
                    const s = r.frameId;
        ...
        case "dismissCompletionList":
            try {
                await chrome.tabs.sendMessage(s.tabId, s, {
                    frameId: s.frameId
                })
            } catch (e) {}
        }
...
chrome.storage.local.get("isDark", (function(e) {
    void 0 !== e.isDark && (g_isDark = e.isDark),
    g_lastToolbarIconImageName && setToolbarIcon(g_lastToolbarIconImageName)
}
)),
setUpEventListners(),
checkForValidOS() ? connectToBackgroundNativeAppAndSetUpListeners() : (chrome.storage.local.get("hideUnsupportedOSPrompt", (function(e) {
...
chrome.windows.onFocusChanged.addListener((async e => {
    if (!g_nativeAppPort || e === chrome.windows.WINDOW_ID_NONE)
        return;
...
chrome.tabs.onRemoved.addListener(( (e, s) => {
    ...
    JSON.stringify(n),
    g_nativeAppPort.postMessage(n),
    g_TabsToStateMap.delete(e)
}
));
try {
    "undefined" != typeof window && window.matchMedia("(prefers-color-scheme: dark)")?.addEventListener("change", (function() {
        setIsDark(getIsDark()),
        setToolbarIcon(g_lastToolbarIconImageName)
    }
    ))
...
chrome.runtime.onConnect.addListener((function(e) {
    e.name,
    "completionList" === e.name ? setUpPortToCompetionList(e) : "popup" === e.name ? setUpPortToPopup(e) : e.name
}
));
